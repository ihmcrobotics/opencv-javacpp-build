...
RUN echo 'set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} "${CMAKE_SOURCE_DIR}/cmake/")' | \
    ...
    cmake .. -DWITH_JAVA=OFF -DCMAKE_SYSTEM_PROCESSOR=aarch64 -DCMAKE_LIBRARY_ARCHITECTURE=aarch64-linux-gnu -DCMAKE_TOOLCHAIN_FILE=../platforms/linux/aarch64-gnu.toolchain.cmake -DCMAKE_BUILD_TYPE=Release -DWITH_CUDA=ON -DCUDA_ARCH_BIN=8.7 -DCUDA_ARCH_PTX="" -DOPENCV_EXTRA_MODULES_PATH=../../opencv_contrib-4.7.0/modules -DCUDA_TOOLKIT_ROOT_DIR=/usr/local/cuda-12.6/targets/aarch64-linux -DCMAKE_LIBRARY_PATH=/usr/local/cuda-12.6/targets/aarch64-linux/lib -DCMAKE_EXE_LINKER_FLAGS="-Wl,-rpath-link,/usr/local/cuda-12.6/targets/aarch64-linux/lib" -DCMAKE_SHARED_LINKER_FLAGS="-Wl,-rpath-link,/usr/local/cuda-12.6/targets/aarch64-linux/lib" -DBUILD_EXAMPLES=OFF -DBUILD_TESTS=OFF && \
    make -j22
